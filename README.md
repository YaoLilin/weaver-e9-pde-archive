...
- 流程归档后生成档案，推送到档案系统
- 支持历史流程批量推送
- 可配置不同流程的推送配置，配置元数据赋值
- 推送记录台账中显示所有推送记录，可查看推送结果与错误信息
- 档案系统可通过OA归档结果反馈接口，反馈归档结果
- 可通过计划任务，定时调用档案系统归档结果查询接口，查询归档结果
- 与契约锁集成，可获取盖章文件与签名信息
- 支持查询归档范围验证是否归档
...
- 如果是契约锁签名文件，则调用契约锁接口查询文件签名信息和获取盖章文件
- 先获取盖章文件，然后再根据盖章文件来获取文件签名信息，有可能盖章文件没有签名信息
